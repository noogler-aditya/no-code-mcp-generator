...
export function ResultsDisplay({ result, onReset }: ResultsDisplayProps) {
    const downloadUrl = result.downloadUrl ? `http://localhost:3001${result.downloadUrl}` : '#';
    ...
    return (
        <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-center mb-10">
                ...
                <p className="text-gray-400">Your MCP server is ready for deployment.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 rounded-2xl overflow-hidden mb-8">
                <div className="p-6 border-b border-white/5">
                    <div className="flex items-start justify-between">
                        <div>
                            <h3 className="text-xl font-semibold text-white mb-1">{result.info.title}</h3>
                            <p className="text-sm text-gray-400">Version {result.info.version}</p>
                        </div>
                ...
                <div className="p-6 grid grid-cols-2 gap-6">
                    <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Total Tools</span>
                        <p className="text-3xl font-bold text-white mt-1">{result.toolCount}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Project Type</span>
            ...
            {/* Next Steps */}
            <div>
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Deployment Pipeline</h3>
...
function StepItem({ icon, title, code, isLink = false }: { icon: React.ReactNode, title: string, code: string, isLink?: boolean }) {
    if (isLink) {
        return (
            <a href={code} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors group">
